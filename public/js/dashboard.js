...
function updateStatistics(stats, total) {
  document.getElementById('totalScheduled').textContent = stats?.total_scheduled || total || 0;
  document.getElementById('totalUnscheduled').textContent = stats?.total_unscheduled || 0;
...
function renderTimetable(data) {
  ...
  const hasData = Object.keys(data).length > 0 && Object.values(data).some(day => day.length > 0);
...
function renderClassCard(cls) {
  ...
  const facultyName = `${cls.faculty_first_name || ''} ${cls.faculty_last_name || ''}`.trim() || 'N/A';
  const roomInfo = cls.room_name ? `${cls.room_name}` : 'N/A';
  const buildingInfo = cls.floor_building ? `${cls.floor_building} - ${cls.floor_name}` : '';
